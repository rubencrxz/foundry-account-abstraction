...
// ZkSync Era imports
import {Transaction, MemoryTransactionHelper} 
...
contract ZkMinimalAccountTest is Test { 
   ...
   function _signTransaction (Transaction memory transaction) internal view returns (Transaction memory) {
      bytes32 unsignedTransactionHash = MemoryTransactionHelper.encodeHash(transaction); // encodeHash already put it in the correct format
   ...
   function _createUnsignedTransaction(address from, uint8 transactionType, address to, uint value, bytes memory data) 
        internal 
        view 
        returns (Transaction memory) {
            uint nonce = vm.getNonce(address(minimalAccount));
